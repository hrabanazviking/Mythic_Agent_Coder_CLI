...
def render_celtic_cross_layout(cards_with_positions):
    """Render the Celtic Cross in a visual layout."""
    # positions: 1=center, 2=crossing, 3=above, 4=below, 5=behind, 6=ahead, 7-10=staff
    # Visual layout:
    #          [3]
    #     [5] [1/2] [6]
    #          [4]
    ...
    def card_cell(card_wp, idx):
        if idx < len(cards_with_positions):
            ...
            if len(name) > 16:
                name = name[:14] + "…"
            return f"[{sc}]{name}[/{sc}]\n[{'ill_dignified' if c.is_reversed else 'dignified'}]{rev}[/{'ill_dignified' if c.is_reversed else 'dignified'}]"
        return ""

    # Build the cross visually
    cross_lines = []
    cross_lines.append(f"              ┌─────────────┐")
    cross_lines.append(f"              │  {card_cell(cards_with_positions, 2):^25s} │")  # Crown (3)
    cross_lines.append(f"              └─────────────┘")
    cross_lines.append(f"  ┌─────────┐  ┌─────────────┐  ┌─────────────┐")
    cross_lines.append(f"  │ {card_cell(cards_with_positions, 4):^15s} │  │  {card_cell(cards_with_positions, 0):^25s} │  │  {card_cell(cards_with_positions, 5):^25s} │")  # Past, Present, Future
    cross_lines.append(f"  └─────────┘  │  {card_cell(cards_with_positions, 1):^25s} │  └─────────────┘")  # Crossing
    cross_lines.append(f"               └─────────────┘")
    cross_lines.append(f"              ┌─────────────┐")
    cross_lines.append(f"              │  {card_cell(cards_with_positions, 3):^25s} │")  # Foundation (4)
    cross_lines.append(f"              └─────────────┘")
